...
class UPDCS extends React.Component{
    constructor()
    {
        super()
    ...
    async HandleSubmit4(event)
   {
    this.setState(
     ...
     await  axios.put('http://localhost:4000/cc/editcourseschedule',{
        ...
        nslot:this.state.nslot,
        nlocation:this.state.nlocation
    }, {
        headers: {
...
<Container>
                    <Row>
                        ...
                        <Col>
                            ...
                            <Form.Group controlId="formBasicEmail">
                                <Form.Label>              </Form.Label>
                                <Form.Control  placeholder="new day" onChange={e => this.setState({ nday: e.target.value })}/>
                                <Form.Text className="text-muted">
                                pleas make sure that it is valid ex.["saturday", "sunday", "monday", "tuesday", "wednesday", "thursday"];
                                </Form.Text>
                            </Form.Group>
                            <Form.Group controlId="formBasicEmail">
                                <Form.Label>                   </Form.Label>
                                <Form.Control type="number"  placeholder="new slot number" onChange={e => this.setState({ nslot: e.target.value })}/>
                                <Form.Text className="text-muted">
                                pleas make sure that it is valid ex. 1
                                </Form.Text>
                            </Form.Group>
                            <Form.Group controlId="formBasicEmail">
                                <Form.Label>                   </Form.Label>
                                <Form.Control  placeholder="new location" onChange={e => this.setState({ nlocation: e.target.value })}/>
                                <Form.Text className="text-muted">
                                pleas make sure that it is valid d4-102
                                </Form.Text>
                            </Form.Group>
